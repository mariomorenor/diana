...
class EDocument(models.AbstractModel):
    ...
    def _info_tributaria_retencion(self, document, access_key, emission_code):
        """
        """
        company = document.company_id
        infoTributaria = {
            ...
            'secuencial': self.name[6:15],
            'dirMatriz': company.street
        }
        return infoTributaria
        
    def _info_tributaria_guia(self, document, access_key, emission_code):
        """
        """
        company = document.company_id
        infoTributaria = {
            ...
            'claveAcceso': access_key,
            'codDoc': self.journal_id.sri_doctype,
            'estab': self.picking_number_total[0:3],
            'ptoEmi': self.picking_number_total[3:6],
            'secuencial': self.picking_number_total[6:15],
            'dirMatriz': company.street
        }
        return infoTributaria
    ...
    def get_access_key(self, name):
        if name == 'account.move' or name == 'account.debit':
           date_doc = self.invoice_date.strftime("%d%m%Y")
           sequence = self.name[8:17]
           establishment = self.name[4:7]
           emission = self.name[0:3]
        if name == 'account.retention':
           date_doc = self.date.strftime("%d%m%Y")
           sequence = self.name[6:15]
           establishment = self.name[3:6]
           emission = self.name[0:3]
        if name == 'stock.picking':
            date_doc = self.date.strftime("%d%m%Y")
            sequence = self.picking_number
            establishment = self.picking_number_total[3:6]
            emission = self.picking_number_total[0:3] 
